...
@router.get(
    ...
    responses={
        ...
        500: {
            ...
            "content": {"application/json": {"example": {"detail": "Error fetching all tournament records: error"}}},
        },
    },
)
def get_all_tournaments(session: SessionDep):
    try:
        ...
        session.rollback()
        raise HTTPException(status_code=500, detail=f"Error fetching all tournament records: {e}")
